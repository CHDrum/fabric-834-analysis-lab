...
@pytest.mark.parametrize("source", ["synthetic_834.edi", "synthetic_malformed_834.edi"])
def test_pipeline_start_posts_one_explicit_parameter_request(source):
    from instructor.run_pipeline import ROOT, start

    bindings = {"synthetic_only": True, "workspace": "00000000-0000-4000-8000-000000000001",
                "pipeline": "00000000-0000-4000-8000-000000000002"}
    source = source if (ROOT / "src" / "parse_834.py").exists() else None
...
def test_pipeline_rejects_unapproved_source_before_network_access():
    from instructor.run_pipeline import start

    bindings = {"synthetic_only": True, "workspace": "00000000-0000-4000-8000-000000000001",
                "pipeline": "00000000-0000-4000-8000-000000000002"}
    client = SimpleNamespace(pages=Mock(), request=Mock())
    with pytest.raises(ValueError, match="approved synthetic"):
        start(client, bindings, "unapproved_834.edi")
